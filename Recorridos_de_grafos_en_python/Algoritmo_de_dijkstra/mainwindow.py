...
from algoritmos import alg_kruskal
from algoritmos import alg_dijkstra
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.ordenar_Id_pushButton.clicked.connect(self.click_ordenar_Id)
        self.ui.ordenar_Distancia_pushButton.clicked.connect(self.click_ordenar_distancia)
        self.ui.ordenar_Velocidad_pushButton.clicked.connect(self.click_ordenar_velocidad)
    ...
    def action_kruskal(self):
        grafo_kruskal = self.grafo_kruskal()
        pen = QPen()
        color = QColor(255, 0, 212)
        pen.setColor(color)

        for vertice in grafo_kruskal.keys():
            origen_x = vertice[0][0]
            origen_y = vertice[0][1]

            destino_x = vertice[1][0]
            destino_y = vertice[1][1]

            pen.setWidth(3)
            self.scene.addEllipse(origen_x, origen_y, 3, 3, pen)
            self.scene.addEllipse(destino_x, destino_y, 3, 3, pen)
            pen.setWidth(2)
            self.scene.addLine(origen_x+1.5, origen_y+1.5, destino_x+1.5, destino_y+1.5, pen)

    def grafo_kruskal(self):
        grafo_velocidad = dict()
        for particula in self.particulas:
            arista_o_d = ((particula.destino_x, particula.destino_y), particula.velocidad*-1)
            arista_d_o = ((particula.origen_x, particula.origen_y), particula.velocidad*-1)

            origen = (particula.origen_x, particula.origen_y)
            if origen in grafo_velocidad:
                grafo_velocidad[origen].append(arista_o_d)
            else:
                grafo_velocidad[origen] = [arista_o_d]
            
            destino = (particula.destino_x, particula.destino_y)
            if destino in grafo_velocidad:
                grafo_velocidad[destino].append(arista_d_o)
            else:
                grafo_velocidad[destino] = [arista_d_o]
        return alg_kruskal(grafo_velocidad)
    ...
    def action_dijkstra(self):
        ...
        x = self.ui.origenX_spinBox.value()
        y = self.ui.origenY_spinBox.value()
        x2 = self.ui.destinoX_spinBox.value()
        y2 = self.ui.destinoY_spinBox.value()
        origen = (x, y)
        destino = (x2, y2)
        recorrido = alg_dijkstra(grafo, origen, destino)
