...
def llenado_automatico(i):
    os.system("cls")
    id = i+1
    num1 = 5#(random.randrange(0, 100))
    num2 = 5#(random.randrange(0, 100))
    operacion = 1#(random.randrange(1, 5))
    tiempo_estimado = 5#(random.randrange(6, 16))

    if((operacion == 4 or operacion == 5) and num2 == 0):
        num2 = 20#(random.randrange(1, 100))
        continuar = (num2==0)
...
def teclas():
    ...
    if(msvcrt.kbhit()):
        ...
        if(tecla == 'i' or tecla == 'I'):
            ...
            llenado_automatico(len(nuevos)+len(terminados)+len(listos)+len(ejecucion)+len(bloqueados))
    ...
    if len(listos) >= 0:
        ...
        while(tiempo_transcurrido<=ejecucion[0].tiempo_estimado):
            ...
            if len(ejecucion) > 0 and ejecucion[0].error != 1:
                ...
                print("{:<15}{:^10}".format("Operación: ", str(obtener_operacion(ejecucion[0]))))
            ...
            for terminado in terminados:
                if(terminado.error == 0):
                    print("{:<15}{:^10}".format(str(terminado.id), str(obtener_resultado(terminado))))
            ...
            if(len(ejecucion) == 1 and tiempo_transcurrido>ejecucion[0].tiempo_estimado and ejecucion[0].error == 0):
                print("{:<15}{:^10}".format(str(ejecucion[0].id), str(obtener_resultado(ejecucion[0]))))
...
def impresion_de_tabla():
    ...
    for i in range(0, len(terminados)):
        ...
        if terminados[i].error == 0:
            ...
            print('{:^10}{:^10}{:^20}{:^10}{:^10}{:^10}{:^10}{:^10}{:^10}{:^10}'.format(str(terminados[i].id), "T", "ERROR", 
            str(terminados[i].tiempo_llegada), 
            str(terminados[i].tiempo_finalizacion), 
            str(terminados[i].tiempo_retorno), 
            str(terminados[i].espera), 
